...
| T008 - Build Shop Page | Implementation complete, visual QA and live verification pending | The typed TanStack Query catalog route, product grid, and designed loading/empty/error states are in place. Live verification requires Shopify configuration and catalog data. |
| T009 - Build Product Detail Page | Implementation complete, visual QA and live verification pending | The typed TanStack Query product route, gallery, variant selection, and non-mutating cart handoff are in place. Live verification requires Shopify configuration plus product and variant data. |
